            ...
            for light in &scene.lights {
                ...
                if in_light(&phit, light, scene, &direction_to_light) {
                    ...
                    if let &SurfaceType::Reflective(reflectivity) = intersection.object.surface() {
                        let reflected_light =
                            reflective_light(&normal, &ray.direction, &phit, scene, depth);
                        colour = colour + (reflected_light * reflectivity);
                        let specular_light =
                            phong_lighting(light, &direction_to_light, ray, &phit, &normal, intersection.object);
                        // let specular_light =
